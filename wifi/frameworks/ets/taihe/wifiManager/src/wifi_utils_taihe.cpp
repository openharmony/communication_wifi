...
::ohos::wifiManager::Ipv6Config MakeIpv6Config(const WifiIpConfig& wifiIpConfig)
{
    std::vector<std::string> vecDns = {
        wifiIpConfig.staticIpAddress.dnsServer1.GetIpv6Address(),
        wifiIpConfig.staticIpAddress.dnsServer2.GetIpv6Address()
    };
    ::taihe::array<taihe::string> resDns = ::taihe::array<taihe::string>(
            taihe::copy_data_t{}, vecDns.data(), vecDns.size());
    std::vector<std::string> vecDomains = {wifiIpConfig.staticIpAddress.domains};
    ::taihe::array<taihe::string> resDomains = ::taihe::array<taihe::string>(
            taihe::copy_data_t{}, vecDomains.data(), vecDomains.size());
    return {
        wifiIpConfig.staticIpAddress.ipAddress.address.GetIpv6Address(),
        wifiIpConfig.staticIpAddress.gateway.GetIpv6Address(),
        wifiIpConfig.staticIpAddress.ipAddress.prefixLength,
        resDns, resDomains
    };
}
...
::ohos::wifiManager::WifiDeviceConfig MakeWifiDeviceConfig(const WifiDeviceConfig& wifiDeviceConfig)
{
    ...
    return {
        ...
        ipType,
        ::taihe::optional<int32_t>(std::in_place_t{},
            wifiDeviceConfig.wifiIpConfig.staticIpAddress.ipAddress.address.family),
        ::taihe::optional<::ohos::wifiManager::IpConfig>(std::in_place_t{},
            MakeIpConfig(wifiDeviceConfig.wifiIpConfig)),
        ::taihe::optional<::ohos::wifiManager::Ipv6Config>(std::in_place_t{},
            MakeIpv6Config(wifiDeviceConfig.wifiIpConfig)),
        eapConfig,
...
void ConfigStaticIp(const ::ohos::wifiManager::WifiDeviceConfig &config, WifiDeviceConfig& cppConfig)
{
    if (bool(config.family)) {
        int family = *(config.family);
        cppConfig.wifiIpConfig.staticIpAddress.ipAddress.address.family = family;
        if (family == IPV6_ADDRESS_TYPE) {
            ConfigStaticIpv6(config, cppConfig);
        } else {
...
void ConfigStaticIpv6(const ::ohos::wifiManager::WifiDeviceConfig &config, WifiDeviceConfig& cppConfig)
{
    const int dnsNum = 2;
    if (bool(config.staticIpv6)) {
        ::ohos::wifiManager::Ipv6Config ipv6Config = *(config.staticIpv6);
        std::string ipAddress = static_cast<std::string>(ipv6Config.ipAddress);
        std::string gateway = static_cast<std::string>(ipv6Config.gateway);
        cppConfig.wifiIpConfig.staticIpAddress.ipAddress.address.SetIpv6Address(
            IpTools::ConvertIpv6AddressToCompleted(ipAddress));
        ...
        cppConfig.wifiIpConfig.staticIpAddress.gateway.SetIpv6Address(
            IpTools::ConvertIpv6AddressToCompleted(gateway));
        cppConfig.wifiIpConfig.staticIpAddress.ipAddress.prefixLength = ipv6Config.prefixLength;

        if (ipv6Config.dnsServers.size() == 0 || ipv6Config.dnsServers.size() > dnsNum) {
            WIFI_LOGE("ConfigStaticIpv6, It needs dns servers or dns too much.");
            return;
        }
        std::string dnsServer0 = static_cast<std::string>(ipv6Config.dnsServers[0]);
        cppConfig.wifiIpConfig.staticIpAddress.dnsServer1.SetIpv6Address(
            IpTools::ConvertIpv6AddressToCompleted(dnsServer0));
        if (ipv6Config.dnsServers.size() == dnsNum) {
            std::string dnsServer1 = static_cast<std::string>(ipv6Config.dnsServers[1]);
            cppConfig.wifiIpConfig.staticIpAddress.dnsServer1.SetIpv6Address(
                IpTools::ConvertIpv6AddressToCompleted(dnsServer1));
        }
    }
}
...
::ohos::wifiManager::WifiScanInfo MakeWifiScanInfo(const WifiScanInfo& scanInfo)
{
    ...
    return {scanInfo.ssid, scanInfo.bssid,
        ...
        isHiLinkNetwork,
        ::taihe::optional<bool>(std::in_place_t{}, scanInfo.isHiLinkProNetwork)
    };
}
