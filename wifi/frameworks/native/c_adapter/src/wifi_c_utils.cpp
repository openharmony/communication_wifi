...
static std::map<ErrCode, WifiErrorCode> g_ErrCodeMap = {
    ...
    {WIFI_OPT_NON_SYSTEMAPP, ERROR_WIFI_UNKNOWN},
    ...
    {WIFI_OPT_GET_ENHANCE_SVC_FAILED, ERROR_WIFI_ENHANCE_SVC},
    {WIFI_OPT_STA_AND_P2P_MAC_SAME, ERROR_P2P_AND_STA_MAC_SAME},
    {WIFI_OPT_P2P_INVALID_PASSWORD, ERROR_P2P_INVALID_PASSWORD}
};
...
WifiErrorCode IpStrToArray(const std::string& str, unsigned int ipAddr[IPV4_ARRAY_LEN]) {
    ...
    for (int i = 0; i != IPV4_ARRAY_LEN && i != (int)vec.size(); ++i) {
        ipAddr[i] = static_cast<size_t>(CheckDataToUint(vec[i]));
    }
    return WIFI_SUCCESS;
}
