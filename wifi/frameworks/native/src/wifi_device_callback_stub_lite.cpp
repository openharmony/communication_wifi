...
NO_SANITIZE("cfi") void WifiDeviceCallBackStub::OnDeviceConfigChanged(ConfigChange state)
{
...
int WifiDeviceCallBackStub::RemoteOnWifiConnectionChanged(uint32_t code, IpcIo *data)
{
    ...
    char *tmpSsid = reinterpret_cast<char *>(ReadString(data, &readLen));
    char *tmpBssid = reinterpret_cast<char *>(ReadString(data, &readLen));
    if (tmpSsid == nullptr || tmpBssid == nullptr) { return -1; }
    info.ssid = tmpSsid;
    info.bssid = tmpBssid;
