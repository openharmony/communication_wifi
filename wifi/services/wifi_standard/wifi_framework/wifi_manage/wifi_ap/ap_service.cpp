...
ErrCode ApService::GetValidChannels(BandType band, std::vector<int32_t> &validChannel)
{
    ...
    if (channelsInfo.size() > 0) {
        ...
        if (it != channelsInfo.end()) {
            ...
            WIFI_LOGI("%{public}s, get valid channel size:%{public}lu, band:%{public}d from WifiSettings.",
    ...
    WIFI_LOGI("%{public}s, get valid channel size:%{public}lu, band:%{public}d from hal service.",
        __func__, validChannel.size(), band);
    return ErrCode::WIFI_OPT_SUCCESS;
}
