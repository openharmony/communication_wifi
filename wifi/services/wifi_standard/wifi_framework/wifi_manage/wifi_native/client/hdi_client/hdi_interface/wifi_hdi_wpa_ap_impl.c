...
WifiErrorNo HdiStartAp(const char *ifaceName, int id)
{
    ...
    if (SetHdiApIfaceName(ifaceName) != WIFI_HAL_OPT_OK) {
        LOGE("HdiStartAp: set ap iface name failed.");
        return WIFI_HAL_OPT_FAILED;
    }

    if (HdiApStart(id, ifaceName) != WIFI_HAL_OPT_OK) {
        LOGE("HdiStartAp: HdiApStart failed.");
        return WIFI_HAL_OPT_FAILED;
    }

    if (RegisterApEventCallback() != WIFI_HAL_OPT_OK) {
...
WifiErrorNo HdiStopAp(int id)
{
    ...
    if (UnRegisterApEventCallback() != WIFI_HAL_OPT_OK) {
        LOGE("HdiStopAp: UnRegisterApEventCallback failed.");
    }

    if (HdiApStop(id) != WIFI_HAL_OPT_OK) {
