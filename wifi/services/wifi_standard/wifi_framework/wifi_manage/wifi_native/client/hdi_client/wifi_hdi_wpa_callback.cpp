    ...
    const struct HdiWpaDisconnectParam *disconectParam, const char* ifName)
{
    ...
    if (cbk.onWpaSsidWrongKey && isPsk &&
        ...
        std::string bssid = std::string(szBssid);
    ...
    const struct HdiWpaAssociateRejectParam *associateRejectParam, const char *ifName)
{
    ...
    if (isWrongPwd && cbk.onWpaSsidWrongKey) {
        ...
        std::string bssidString = std::string(bssid);
        cbk.onWpaSsidWrongKey(bssidString);
    ...
    const struct HdiWpaAuthRejectParam *authRejectParam, const char *ifName)
{
    ...
    if (cbk.onWpaSsidWrongKey && statusCode == Wifi80211StatusCode::WLAN_STATUS_CHALLENGE_FAIL &&
        ...
        std::string bssid1 = std::string(bssid);
        cbk.onWpaSsidWrongKey(bssid1);
        OHOS::Wifi::WriteAuthFailHiSysEvent("WRONG_PSWD");
        return 0;
    }
    return 0;
