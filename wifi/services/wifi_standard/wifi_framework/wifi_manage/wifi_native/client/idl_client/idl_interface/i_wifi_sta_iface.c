...
WifiErrorNo SetBgLimitMode(int mode)
{
    ...
    Context *context = client->context;
    WriteBegin(context, 0);
    WriteFunc(context, "SetBgLimitMode");
    WriteInt(context, mode);
    WriteEnd(context);
    if (RpcClientCall(client, "SetBgLimitMode") != WIFI_IDL_OPT_OK) {
        return WIFI_IDL_OPT_FAILED;
    }
    int result = WIFI_IDL_OPT_FAILED;
    ReadInt(context, &result);
...
WifiErrorNo SetBgLimitIdList(int *idArray, int size, int type)
{
    ...
    Context *context = client->context;
    WriteBegin(context, 0);
    WriteFunc(context, "SetBgLimitIdList");
    WriteInt(context, size);
    for (int i = 0; i < size; i++) {
        WriteInt(context, idArray[i]);
    }
    WriteInt(context, type);
    WriteEnd(context);
    if (RpcClientCall(client, "SetBgLimitIdList") != WIFI_IDL_OPT_OK) {
        return WIFI_IDL_OPT_FAILED;
    }
    int result = WIFI_IDL_OPT_FAILED;
    ReadInt(context, &result);
