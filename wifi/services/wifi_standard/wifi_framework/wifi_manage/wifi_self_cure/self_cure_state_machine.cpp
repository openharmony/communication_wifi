...
SelfCureStateMachine::~SelfCureStateMachine()
{
    ...
    ParsePointer(pWifi7SelfCureState_);
...
void SelfCureStateMachine::BuildStateTree()
{
    ...
    StatePlus(pWifi7SelfCureState_, pDisconnectedMonitorState_);
...
ErrCode SelfCureStateMachine::InitSelfCureStates()
{
    ...
    pWifi7SelfCureState_ = new (std::nothrow)Wifi7SelfCureState(this);
    tmpErrNumber += JudgmentEmpty(pWifi7SelfCureState_);
...
void SelfCureStateMachine::ConnectedMonitorState::HandleRssiLevelChange(InternalMessagePtr msg)
{
    ...
    pSelfCureStateMachine_->lastSignalLevel_ = pSelfCureStateMachine_->GetCurSignalLevel();
}
...
void SelfCureStateMachine::ConnectedMonitorState::HandleArpDetectionFailed(InternalMessagePtr msg)
{
    ...
    if (pSelfCureStateMachine_->NeedWifi7SelfCure(lastConnectedBssid_)) {
        return;
    }
...
{
    if (pSelfCureStateMachine_->lastSignalLevel_ <= SIGNAL_LEVEL_1) {
...
void SelfCureStateMachine::InternetSelfCureState::HandleArpFailedDetected(InternalMessagePtr msg)
{
    ...
    if (pSelfCureStateMachine_->NeedWifi7SelfCure(currentBssid_)) {
        return;
    }
...
void SelfCureStateMachine::NoInternetState::HandleArpFailedDetected(InternalMessagePtr msg)
{
    ...
    std::string currentBssid = pSelfCureStateMachine_->GetCurrentBssid();
    if (pSelfCureStateMachine_->NeedWifi7SelfCure(currentBssid)) {
        return;
    }
...
SelfCureStateMachine::Wifi7SelfCureState::Wifi7SelfCureState(SelfCureStateMachine *selfCureStateMachine)
    : State("Wifi7SelfCureState"), pSelfCureStateMachine_(selfCureStateMachine)
{
    WIFI_LOGI("Wifi7SelfCureState construct success");
}

SelfCureStateMachine::Wifi7SelfCureState::~Wifi7SelfCureState() {}
void SelfCureStateMachine::Wifi7SelfCureState::GoInState()
{
    WIFI_LOGI("Wifi7SelfCureState GoInState");
}

void SelfCureStateMachine::Wifi7SelfCureState::GoOutState()
{
    WIFI_LOGI("Wifi7SelfCureState GoOutState");
}
...
void SelfCureStateMachine::Wifi7SelfCureState::HandleWifi7ArpFailMsg()
{
    WifiLinkedInfo linkedInfo;
    WifiConfigCenter::GetInstance().GetLinkedInfo(linkedInfo);
    if (pSelfCureStateMachine_->lastSignalLevel_ >= SigLevel::SIG_LEVEL_3 &&
        !pSelfCureStateMachine_->CanArpReachable()) {
        ExecuteWifi7ArpFailSelfCure(linkedInfo);
    }
    pSelfCureStateMachine_->SwitchState(pSelfCureStateMachine_->pConnectedMonitorState_);
}

void SelfCureStateMachine::Wifi7SelfCureState::ExecuteWifi7ArpFailSelfCure(const WifiLinkedInfo &info)
{
    ...
    auto iter = wifi7BlackListCache.find(info.bssid);
    ...
    } else if (iter->second.actionType == ACTION_TYPE_MLD) {
        WIFI_LOGI("ExecuteWifi7ArpFailSelfCure start wifi7 non_mld back off");
        pSelfCureStateMachine_->HandleWifi7WithoutMldBackoff(info);
        IStaService *pStaService = WifiServiceManager::GetInstance().GetStaServiceInst(0);
        if (pStaService == nullptr) {
            WIFI_LOGE("ExecuteWifi7ArpFailSelfCure Get pStaService failed!");
...
void SelfCureStateMachine::PeriodicArpDetection()
{
    ...
    if (!CanArpReachable()) {
        ...
        if (arpDetectionFailedCnt_ == ARP_DETECTED_FAILED_COUNT) {
            ...
            MessageExecutedLater(WIFI_CURE_CMD_PERIODIC_ARP_DETECTED, fastArpDetectTime_);
            return;
        }
    } else {
...
int SelfCureStateMachine::GetWifi7SelfCureType(WifiLinkedInfo &info)
{
    std::vector<WifiScanInfo> scanResults;
    WifiConfigCenter::GetInstance().GetWifiScanConfig()->GetScanInfoList(scanResults);
    ...
    WIFI_LOGI("GetWifi7SelfCureType scanRssi %{public}d, wifiCategory: %{public}d",
        scanRssi, static_cast<int>(wifiCategory));
    if ((wifiCategory == WifiCategory::WIFI7 || wifiCategory == WifiCategory::WIFI7_PLUS)
        && scanRssi >= MIN_WIFI7_SELFCURE_RSSI) {
...
void SelfCureStateMachine::ShouldTransToWifi7SelfCure(WifiLinkedInfo &info)
{
    ...
    int wifi7SelfCureType = GetWifi7SelfCureType(info);
...
std::set<std::string> SelfCureStateMachine::GetCandidateBssidsForBlacklist(const WifiLinkedInfo &info)
{
    std::set<std::string> bssids;
    bssids.insert(info.bssid);
    if (info.ssid.empty()) {
        WIFI_LOGE("%{public}s: ssid is empty.", __FUNCTION__);
        return bssids;
    }
    std::vector<WifiScanInfo> scanResults;
    WifiConfigCenter::GetInstance().GetWifiScanConfig()->GetScanInfoList(scanResults);
    
    std::vector<std::pair<std::string, int>> bssidRssiList;
    for (const auto &scanInfo : scanResults) {
        if (scanInfo.ssid == info.ssid && !scanInfo.bssid.empty() && scanInfo.rssi >= MIN_WIFI7_SELFCURE_RSSI) {
            bssidRssiList.push_back({scanInfo.bssid, scanInfo.rssi});
        }
    }
    
    std::sort(bssidRssiList.begin(), bssidRssiList.end(),
    ...
    WIFI_LOGI("GetCandidateBssidsForBlacklist: ssid %{public}s found %{public}zu bssids",
        SsidAnonymize(info.ssid).c_str(), bssids.size());
    return bssids;
}
...
bool SelfCureStateMachine::NeedWifi7SelfCure(const std::string &bssid)
{
    WifiLinkedInfo linkedInfo;
    WifiConfigCenter::GetInstance().GetLinkedInfo(linkedInfo);
    ...
    std::map<std::string, WifiCategoryBlackListInfo> wifi7BlackListCache;
    WifiConfigCenter::GetInstance().GetWifiCategoryBlackListCache(EVENT_BE_BLA_LIST, wifi7BlackListCache);
    auto iter = wifi7BlackListCache.find(bssid);
    if (iter != wifi7BlackListCache.end()) {
        WIFI_LOGI("NeedWifi7SelfCure current network is mlo back off");
        return false;
    }
...
bool SelfCureStateMachine::IsSoftApSsidSameWithWifi(const HotspotConfig& curApConfig)
{
    ...
    bool isSameSecurityType = (config.keyMgmt == "WPA2-PSK" || config.keyMgmt == "WPA-PSK");
...
void SelfCureStateMachine::HandleWifi7WithoutMldBackoff(const WifiLinkedInfo &info)
{
    if (info.bssid.empty()) {
        WIFI_LOGE("%{public}s: bssid is empty.", __FUNCTION__);
        return;
    }
...
void SelfCureStateMachine::HandleWifi7MldBackoff(const WifiLinkedInfo &info)
{
    if (info.bssid.empty()) {
        WIFI_LOGE("%{public}s: bssid is empty.", __FUNCTION__);
        return;
