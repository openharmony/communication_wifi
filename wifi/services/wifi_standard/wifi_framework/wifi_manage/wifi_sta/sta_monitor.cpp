...
void StaMonitor::OnWpaCustomEapNotifyCallBack(const std::string &notifyParam)
{
    ...
    for (size_t i = 0; i < paramSize - 1; i++) {
        if (!CheckDataLegalContainZero(vecEapDatas[i])) {
            WIFI_LOGE("%{public}s notifyParam %{public}zu is not number", __FUNCTION__, i);
            return;
        }
    }
    WpaEapData wpaEapData;
    wpaEapData.msgId = *CheckDataLegalContainZero(vecEapDatas[EAP_DATE_ZERO]);
    wpaEapData.code = *CheckDataLegalContainZero(vecEapDatas[EAP_DATE_ONE]);
    wpaEapData.type = *CheckDataLegalContainZero(vecEapDatas[EAP_DATE_TWO]);
    wpaEapData.bufferLen = *CheckDataLegalContainZero(vecEapDatas[EAP_DATE_THREE]);
