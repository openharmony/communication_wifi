...
void StaStateMachine::DhcpResultNotify::OnSuccess(int status, const char *ifname, DhcpResult *result)
{
    ...
    pStaStateMachine->pDhcpResultNotify->OnSuccessDhcpResult(status, ifname, result);
}
...
void StaStateMachine::DhcpResultNotify::OnFailed(int status, const char *ifname, const char *reason)
{
    if (pStaStateMachine == nullptr) {
        WIFI_LOGE("StaStateMachine DhcpResultNotify OnFailed pStaStateMachine nullptr.");
    }
    pStaStateMachine->pDhcpResultNotify->OnFailedDhcpResult(status, ifname, reason);
}
