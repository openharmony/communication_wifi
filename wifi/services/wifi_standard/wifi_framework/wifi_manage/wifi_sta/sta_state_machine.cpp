...
void StaStateMachine::DealConnectToUserSelectedNetwork(InternalMessagePtr msg)
{
    ...
    if (networkId == linkedInfo.networkId) {
        ...
        if (linkedInfo.connState == ConnState::CONNECTING &&
            ...
            WIFI_LOGI("This network is connecting and does not need to be reconnected m_instId = %{public}d", m_instId);
            return;
        }
    }
...
bool StaStateMachine::SetRandomMac(int networkId, const std::string &bssid)
{
    ...
    if (MacAddress::IsValidMac(currentMac.c_str())) {
        if (lastMac != currentMac && m_instId == 0) {
            if (WifiStaHalInterface::GetInstance().SetConnectMacAddr(
                WifiConfigCenter::GetInstance().GetStaIfaceName(m_instId), currentMac) != WIFI_HAL_OPT_OK) {
                LOGE("set Mac [%{public}s] failed.", MacAnonymize(currentMac).c_str());
                return false;
            }
        }
        if (m_instId == 1) {
            currentMac.replace(9, 2, "8e");
            LOGI("SetRandomMac currentMac 8e, use %{public}s, instId is %{public}d", currentMac.c_str(), m_instId);
            if (WifiStaHalInterface::GetInstance().SetConnectMacAddr(
                WifiConfigCenter::GetInstance().GetStaIfaceName(m_instId), currentMac) != WIFI_HAL_OPT_OK) {
                HILOGI("set wlan1 Mac [%{public}s] failed", currentMac.c_str());
            } else {
                HILOGI("set wlan1 Mac [%{public}s] success", currentMac.c_str());
            }
        }
        LOGI("SetRandomMac randomMac, use %{public}s, instId is %{public}d", realMac.c_str(), m_instId);
        WifiConfigCenter::GetInstance().SetMacAddress(currentMac, m_instId);
