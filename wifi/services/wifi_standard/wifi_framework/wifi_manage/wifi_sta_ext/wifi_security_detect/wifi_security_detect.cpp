...
const std::string SETTINGS_DATASHARE_URI = "datashare:///com.ohos.settingsdata/entry/settingsdata/SETTINGSDATA?Proxy=true";
...
bool WifiSecurityDetect::DealStaConnChanged(OperateResState state, const WifiLinkedInfo &info, int instId)
{
    ...
    if (state == OperateResState::CONNECT_AP_CONNECTED) {
        WIFI_LOGI("WifiSecurityDetect network connected %{public}d", info.networkId);
        ...
        WIFI_LOGI("WifiSecurityDetect network not connected %{public}d", state);
        return false;
    }
}
...
bool WifiSecurityDetect::SecurityDetectTime(const int &networkId)
{
    ...
    WifiSettings& setting = WifiSettings::GetInstance();
    if (setting.GetDeviceConfig(networkId, config) != 0) {
    ...
    auto hours = std::chrono::duration_cast<std::chrono::hours>(DetectTime - std::chrono::system_clock::from_time_t(config.lastConnectTime));
    if (hours.count() > 24 ) {
...
bool WifiSecurityDetect::SecurityDetectResult(const std::string &devId, uint32_t modelId,
...
void WifiSecurityDetect::WifiConnectConfigParma(const WifiLinkedInfo &info, Json::Value &root)
{
    WifiDeviceConfig config;
    WifiSettings& setting = WifiSettings::GetInstance();
    if (setting.GetDeviceConfig(info.networkId, config) != 0) {
