...
void WifiStaManager::DealStaConnChanged(OperateResState state, const WifiLinkedInfo &info, int instId)
{
    ...
    if (state == OperateResState::DISCONNECT_DISCONNECTED && info.connTriggerMode !=
...
void WifiStaManager::InstallHpfAndSetSuspendMode(int instId)
{
    ...
    filterData.eventCode = screenState;
    WifiManager::GetInstance().InstallPacketFilterProgram(filterData, instId);
#endif
    if (instId == INSTID_WLAN0) {
