...
static bool FilterWifiConfigKey(const std::string &key)
{
    std::set<std::string> filterKey;
    filterKey.insert("version");
    if (filterKey.find(key) != filterKey.end()) {
        return true;
    }
    return false;
}
