...
HWTEST_F(AppNetworkSpeedLimitServiceTest, HandleProcessCreatedEventTest1, TestSize.Level1)
{
...
HWTEST_F(AppNetworkSpeedLimitServiceTest, HandleProcessStateChangedEventTest1, TestSize.Level1)
{
    AppExecFwk::ProcessData data;
    data.bundleName = "xxx";
    data.state = AppExecFwk::AppProcessState::APP_STATE_FOREGROUND;
    AppNetworkSpeedLimitService::GetInstance().m_bgLimitRecordMap = {{4, 3}};
    AppNetworkSpeedLimitService::GetInstance().HandleProcessStateChangedEvent(data);
    EXPECT_EQ(data.state, AppExecFwk::AppProcessState::APP_STATE_FOREGROUND);
}
...
HWTEST_F(AppNetworkSpeedLimitServiceTest, BackGroundAppChangedActionTest1, TestSize.Level1)
{
    ...
    AsyncParamInfo info;
    info.bundleName = "xxx";
    AppNetworkSpeedLimitService::GetInstance().BackgroundAppChangedAction(info);
    EXPECT_EQ(AppNetworkSpeedLimitService::GetInstance().m_isWifiConnected, false);
}

HWTEST_F(AppNetworkSpeedLimitServiceTest, BackGroundAppChangedActionTest2, TestSize.Level1)
{
    AppNetworkSpeedLimitService::GetInstance().m_isWifiConnected = true;
...
HWTEST_F(AppNetworkSpeedLimitServiceTest, UpdateForegroundAppConfigsTest01, TestSize.Level1)
{
    AppNetworkSpeedLimitService::GetInstance().UpdateForegroundAppConfigs();
    EXPECT_FALSE(g_errLog.find("service is null")!=std::string::npos);
}

HWTEST_F(AppNetworkSpeedLimitServiceTest, UpdateBackgroundAppConfigsTest01, TestSize.Level1)
{
    const int enable = 1;
...
HWTEST_F(AppNetworkSpeedLimitServiceTest, SendLimitCmd2Drv_NonSpecialWifi, TestSize.Level1)
{
...
HWTEST_F(AppNetworkSpeedLimitServiceTest, SendLimitCmd2Drv_SpecialWifi, TestSize.Level1)
{
    AppNetworkSpeedLimitService::GetInstance().isVpnConnected_ = false;
    AppNetworkSpeedLimitService::GetInstance().m_bgLimitRecordMap.clear();
    ...
    sleep(1);
    int mode = AppNetworkSpeedLimitService::GetInstance().m_bgLimitRecordMap[BG_LIMIT_CONTROL_ID_KEY_FG_APP];
    EXPECT_EQ(BG_LIMIT_LEVEL_12, mode);
}
