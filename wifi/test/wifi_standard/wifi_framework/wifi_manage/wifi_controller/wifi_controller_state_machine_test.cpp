...
{
    // Test HandleWifiToggleChangeInDisabledState vid CMD_SCAN_ALWAYS_MODE_CHANGED
    ...
    WifiConfigCenter::GetInstance().SetWifiToggledState(false, instId);
    WifiSettings::GetInstance().SetScanOnlySwitchState(0, 0);
    WifiConfigCenter::GetInstance.SetWifiDetailStateStopState(true);
...
HWTEST_F(WifiControllerMachineTest, HandleWifiToggleChangeInDisabledStateTest02, TestSize.Level1)
{
    ...
    // and ShouldEnableWifi return true (WiFi can be enabled)
    int instId = 0£»
    InternalMessagePtr msg = std::make_shared<InternalMessage>();
    msg->SetMessageName(CMD_WIFI_TOGGLED);
...
HWTEST_F(WifiControllerMachineTest, HandleWifiToggleChangeInDisabledStateTest03, TestSize.Level1)
{
    ...
    // and ShoudEnableWifi returns true but GetWifiRole returns ROLE_UNKNOW
    int instId = 0;
    InternalMessagePtr msg = std::make_shared<InternalMessage>();
    msg->SetMessageName(CMD_SCAN_ALWAYS_MODE_CHANGED);
    msg->SetParam1(1);
...
{
    // Test HandleEnableStateManagerExist with GetWifiStopState returns true (should early return)
    int id = 0;
    int isOpen = 1;
    InternalMessagePtr msg = std::make_shared<InternalMessage>();
    msg->SetMessageName(CMD_WIFI_TOGGLED);
...
HWTEST_F(WifiControllerMachineTest, HandleEnableStateManagerExistTest04, TestSize.Level1)
{
    ...
    // Add a concreate manager so the IdExist check passes
    ...
    EXPECT_FALSE(g_errLog.find("HandleEnableStateManagerExist: msg is null") != std::string::npos);
}
}
}
