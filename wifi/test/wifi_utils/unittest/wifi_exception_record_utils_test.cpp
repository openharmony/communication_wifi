...
public:
    ...
    void SetUp(){ unlink(TEST_FILE_PATH);}
    void TearDown(){ unlink(TEST_FILE_PATH);}

    WifiExceptionRecord MakeRecord(const srd::string& ssid,ExceptionReason reason,
...
HWTEST_F(WifiExceptionRecordUtilsTest,AddException_, TestSize.Level10)
{
...
HWTEST_F(WifiExceptionRecordUtilsTest,AddException_MergeSameReasonDetail, TestSize.Level10)
{
    WifiExceptionRecordUtils utils;
    auto rec1 = MakeRecord("AP1",ExceptionReason::DHCP_GRT_IP_TIMEOUT, 1000,-1,"timeout");
    auto rec2 = MakeRecord("AP2",ExceptionReason::DHCP_GRT_IP_TIMEOUT, 2000,-1,"timeout");
...
HWTEST_F(WifiExceptionRecordUtilsTest,AddException_DifferentReasonsNotMerged, TestSize.Level10)
{
    WifiExceptionRecordUtils utils;
    auto rec1 = MakeRecord("AP1",ExceptionReason::DHCP_CONNECTION_FAIL, 1000,-1,"a");
    auto rec2 = MakeRecord("AP2",ExceptionReason::DHCP_GRT_IP_TIMEOUT, 2000,-1,"b");
...
HWTEST_F(WifiExceptionRecordUtilsTest,AddException_PerGroupLimit10, TestSize.Level10)
{
    WifiExceptionRecordUtils utils;
    for (int i = 0; i<15; i++){
        auto rec = MakeRecord("AP1", static_cast<ExceptionReason>(101+i),1000+i,i,"x");
        utils.AddException(rec);
    }
    std::vector<ApGroup>groups;
    utils.GetAllExceptions(groups);
    ASSERT_EQ(groups.size(),1u);
    EXPECT_LE(groups[0].faults.size(),10u);
}

HWTEST_F(WifiExceptionRecordUtilsTest,AddException_GlobalLimit50, TestSize.Level10)
{
    WifiExceptionRecordUtils utils;
    for(int g=0;g<6;g++){
        for(int i= 0;i<10;i++){
            std::string ssid = "AP" +std::to_string(g);
            auto rec = MakeRecord(ssid,static_cast<ExceptionReason>(101+i),1000+g*10+i,i,"x");
            utils.AddException(rec);
        }
    }
    std::vector<ApGroup>groups;
...
HWTEST_F(WifiExceptionRecordUtilsTest,GetAllExceptions_EmptyFile, TestSize.Level10)
{
    WifiExceptionRecordUtils utils;
    std::vector<ApGroup>groups;
    utils.GetAllExceptions(groups);
    EXPECT_EQ(utils.GetAllExceptions(groups), 0);
    EXCEPT_TRUE(groups.empty());
}

HWTEST_F(WifiExceptionRecordUtilsTest,GetAllExceptions_MultipleGroups, TestSize.Level10)
{
    WifiExceptionRecordUtils utils;
    utils.AddException("AP_A",ExceptionReason::DHCP_CONNECTION_FAIL, 1000,-1,"a");
    utils.AddException("AP_B",ExceptionReason::DHCP_GRT_IP_TIMEOUT, 2000,-1,"b");
    std::vector<ApGroup> groups;
    utils.GetAllExceptions(groups);
    EXPECT_EQ(groups.size(),2u);
}

HWTEST_F(WifiExceptionRecordUtilsTest,ClearExceptions_ClearsAll, TestSize.Level10)
{
    WifiExceptionRecordUtils utils;
...
HWTEST_F(WifiExceptionRecordUtilsTest,ReasonToString_AllReasons, TestSize.Level10)
{
    WifiExceptionRecordUtils utils;
...
HWTEST_F(WifiExceptionRecordUtilsTest,CategoryToString_AllCategories, TestSize.Level10)
{
    WifiExceptionRecordUtils utils;
    EXPECT_EQ(utils.CategoryToString(ExceptionReason::DHCP_CONNECTION_FAIL), "DHCP");
}
HWTEST_F(WifiExceptionRecordUtilsTest,FormatTime_BasicFormat, TestSize.Level10)
{
    WifiExceptionRecordUtils utils;
